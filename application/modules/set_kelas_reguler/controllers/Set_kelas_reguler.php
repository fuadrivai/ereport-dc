...
class Set_kelas_reguler extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->sespre = $this->config->item('session_name_prefix');

        $this->d['admlevel'] = $this->session->userdata($this->sespre.'level');
        $this->d['url'] = "set_kelas_reguler";
    ...
    public function edit($id) {
        $q = $this->db->query("SELECT id, nama FROM m_siswa a 
                                ...
                                (SELECT id_siswa FROM t_kelas_siswa_reguler WHERE ta = ".$this->d['ta'].") 
                                ORDER BY id ASC");

        
        
        $t = $this->db->query("SELECT ta from t_kelas_siswa_reguler where ta != '".$this->d['ta']."' order by ta desc limit 1  ")->row_array();
    ...
    public function simpan() {
        ...
        $query = "INSERT IGNORE INTO t_kelas_siswa_reguler (id_kelas, id_siswa, ta) VALUES ".implode(", ", $teks_val).";";
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM t_kelas_siswa_reguler WHERE id = '$id'");
