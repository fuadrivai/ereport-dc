    ...
    public function ambil_siswa($kelas) {
        ...
        if ($jenis == "h") {
           ...
           $ambil_nilai = $this->db->query("SELECT 
            ...
            t_kelas_siswa_reguler.id_kelas
        FROM m_siswa
        LEFT JOIN t_nilai ON m_siswa.id = t_nilai.id_siswa
            AND t_nilai.jenis = 'c'
            AND t_nilai.id_guru_mapel = '$id_guru_mapel'
            
        LEFT JOIN t_kelas_siswa_reguler ON m_siswa.id = t_kelas_siswa_reguler.id_siswa
        WHERE (t_nilai.jenis = 'c' OR t_nilai.jenis IS NULL)
            AND t_kelas_siswa_reguler.ta = '".$this->d['tahun']."'
            AND t_kelas_siswa_reguler.id_kelas = $kelas")->result_array();
            //j($ambil_nilai);
            //exit;
            //echo $this->db->last_query();
            //exit;
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            if ($p['jenis'] == 'c'){
                ...
                if ($cek > 0) {
                    $jumlah_sudah ++;
                    $this->db->query("UPDATE t_nilai SET catatan = '$s', catatan_mid = '".$p['nilai_mid'][$i]."' WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_mapel_kd = '".$p['id_mapel_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."'");
                    //$queri[] = "update";
                } else {
                    $this->db->query("INSERT INTO t_nilai (tasm,jenis, id_guru_mapel, id_mapel_kd, id_siswa, catatan, catatan_mid) VALUES ('".$this->d['tasm']."', '".$p['jenis']."', '".$p['id_guru_mapel']."', '".$p['id_mapel_kd']."', '".$p['id_siswa'][$i]."', '".$s."', '".$p['nilai_mid'][$i]."')");
                    //$queri[] = "add";
                }
            }else{
                $cek = $this->db->query("SELECT id FROM t_nilai WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_mapel_kd = '".$p['id_mapel_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."' AND tasm = '".$this->d['tasm']."'")->num_rows();
