    ...
    function view_kd(id, nama) {
        ...
        if (id == 0) {
            ...
            $.getJSON(base_url+"<?php echo $url; ?>/ambil_siswa/"+id, function(data) {
                ...
                html = '<table class="table table-bordered"><thead><tr><th width="10%" rowspan="2">No</th><th width="40%" rowspan="2">Nama</th><th width="50%" colspan="2">Nilai</th></tr><tr><th width="50%">Link Report</th></thead><tbody>';
                var i = 1;
                $.each(data.data, function(k, v) {
                    html += '<tr><td>'+i+'</td><td>'+v.nmsiswa+'<input name="id_siswa[]" type="hidden" value="'+v.ids+'"></td>';
                        var ides = v.desk == "" ? "" : v.desk;
                    html += '<td><input name="nilai_d[]" type="text" class="form-control input-sm desk" id="desk_'+i+'" value="'+ides+'"></td></tr>';
                    i++;
                }); 
