...
class Login extends CI_Controller {
    ...
    public function do_loginapi() {
        ...
        if($j_cek == 1) {
            ...
            // Add a redirect
            // Add a redirect
            header("Location: https://report.mhis.link/bintaro/dc/lihat_raport");
            exit;
        } else {	
			$d['status'] = "gagal";
    ...
    public function do_login() {
        ...
        if($j_cek == 1) {
            ...
            if ($level == "guru") {
                $cek_is_wali_kelas = $this->db->query("SELECT a.id_kelas, b.nama nmkelas FROM t_walikelas a INNER JOIN m_kelas b ON a.id_kelas = b.id WHERE a.tasm = '".$this->d['ta']."' AND a.id_guru = '".$d_cek->konid."'")->row_array();
                $cek_is_wali_kelas_reguler = $this->db->query("SELECT a.id_kelas, b.nama nmkelas FROM t_walikelas_reguler a INNER JOIN m_kelas b ON a.id_kelas = b.id WHERE a.tasm = '".$this->d['ta']."' AND a.id_guru = '".$d_cek->konid."'")->row_array();
                $detil_nama = $this->db->query("SELECT nama, nip, jk FROM m_guru WHERE id = '".$d_cek->konid."'")->row();
                if (!empty($cek_is_wali_kelas)) {
                    $data = array(
                                $this->sespre.'id' => $d_cek->id,
                                ...
                                $this->sespre.'nip' => $detil_nama->nip,
                                $this->sespre.'walikelas' => array("is_wali"=>true, "id_walikelas"=>$cek_is_wali_kelas['id_kelas'],"nama_walikelas"=>$cek_is_wali_kelas['nmkelas']),
                                $this->sespre.'walikelas_reguler' => array("is_wali"=>true, "id_walikelas"=>$cek_is_wali_kelas_reguler['id_kelas'],"nama_walikelas"=>$cek_is_wali_kelas_reguler['nmkelas'])
                                );  
                } else {
                    $data = array(
                                $this->sespre.'id' => $d_cek->id,
