...
class N_catatan_kelompok extends CI_Controller
{
    function __construct()
    {
        ...
        $this->d['id_kelas_reguler'] = $wali_reguler['id_walikelas'];
    ...
    {
        $id_guru = $this->session->userdata($this->sespre . "konid");
        $this->d['siswa_kelas'] = $this->db->query("SELECT 
        a.id_siswa, b.nama, IFNULL(c.catatan_mid, '') as catatan_mid, IFNULL(c.catatan_final, '') as catatan_final, d.id_guru as id_guru
        FROM t_kelas_siswa_reguler a
        INNER JOIN m_siswa b ON a.id_siswa = b.id
        INNER JOIN t_walikelas_reguler d ON a.id_kelas = d.id_kelas
        LEFT JOIN t_catatan_projek c ON a.id_siswa = c.id_siswa AND c.ta = '" . $this->d['tasm'] . "' AND c.id_kelompok = '" . $id . "'
        WHERE d.id_guru = '" . $id_guru . "' AND a.ta = '" . $this->d['ta'] . "'"
        )->result_array();
    ...
    public function import($bawa,$id_kelompok)
    {
        ...
        LEFT JOIN t_kelas_siswa_reguler c ON CONCAT(a.id_siswa,LEFT(a.ta,4)) = CONCAT(c.id_siswa,c.ta)
    ...
    public function import_nilai($id_kelompok)
    {
        ...
        $q_siswa_kelas = $this->db->query("SELECT a.id_siswa FROM t_kelas_siswa_reguler a WHERE a.id_kelas = '" . $id_kelas . "' AND a.ta = '" . $this->d['ta'] . "'");
