<div class="row">
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="header">
                <h5 class="title">Areas of Coverage &raquo; <?php echo $detil_mp['nmmapel']." - ".$detil_mp['nmkelas']; ?></h5>
            </div>
            <div class="content">
                <ul class="list-group" id="list_kd">
                    <li class="list-group-item" onclick="return view_kd(<?php echo $detil_mp['id_mapel'].", ".$detil_mp['id_kelas']; ?>,'c');"><a href="#"><i class="fa fa-chevron-right"></i>  Catatan Midterm</a></li>
                    <li class="list-group-item" onclick="return view_kd(<?php echo $detil_mp['id_mapel'].", ".$detil_mp['id_kelas']; ?>,'c_f');"><a href="#"><i class="fa fa-chevron-right"></i>  Catatan Final</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    function view_kd(id, kelas, jenis='h') {
        ...
        if (id == 0 && kelas == 0) {
            ...
            $.getJSON(base_url+"<?php echo $url; ?>/ambil_siswa/"+kelas+"/"+id+"/"+jenis, function(data) {
                ...
                html = '<table class="table table-condensed table-bordered"><thead><tr><th width="5%">No</th><th width="15%">Nama</th><th width="35%">Areas of Improvement</th><th width="35%">Follow Up and Recommendation</th></tr></thead><tbody>';
                var i = 1;
                $.each(data.data, function(k, v) {
                    html += '<tr><td>'+i+'</td><td>'+v.nama+'</td><td><input name="id_siswa[]" type="hidden" value="'+v.ids+'"><textarea name="nilai_mid[]" class="form-control input-sm" value="'+stripTags(v.nilai_mid)+'" required>'+stripTags(v.nilai_mid)+'</textarea></td><td><textarea name="nilai[]" class="form-control input-sm" value="'+stripTags(v.nilai)+'" required>'+stripTags(v.nilai)+'</textarea></td></tr>';
                    i++;
                }); 
