   ...
   public function import($bawa) {
    ...
    public function ambil_siswa($kelas) {
        ...
        WHERE (t_nilai_cat.jenis = 'c' OR t_nilai_cat.jenis IS NULL)
            AND t_kelas_siswa.id_kelas = $kelas")->result_array();
            //j($ambil_nilai);
            //exit;
            //echo $this->db->last_query();
            //exit;
        ...
        FROM m_siswa
        LEFT JOIN t_nilai_cat ON m_siswa.id = t_nilai_cat.id_siswa
            AND t_nilai_cat.jenis = 'c_f'
            AND t_nilai_cat.id_guru_mapel = '$id_guru_mapel'
            AND t_nilai_cat.tasm = '".$this->d['tasm']."'
        LEFT JOIN t_kelas_siswa ON m_siswa.id = t_kelas_siswa.id_siswa
        AND t_kelas_siswa.ta = '".$this->d['tahun']."'
        WHERE (t_nilai_cat.jenis = 'c_f' OR t_nilai_cat.jenis IS NULL)
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            ...
            $p['nilai_mid'][$i] = $this->typography->auto_typography($p['nilai_mid'][$i], TRUE);
            $s = $this->typography->auto_typography($s, TRUE);
            if ($cek > 0) {
                $jumlah_sudah ++;
    ...
    public function index($id) {
    	...
    	$this->d['p'] = "list";
    	$this->d['idGuru'] = $this->session->userdata('id_guru_mapel');
        $this->load->view("template_utama", $this->d);
    }
}
