...
class N_keterampilan extends CI_Controller {
    ...
    public function import($bawa) {
        ...
        $q_nilai_harian = $this->db->query("SELECT 
                                ...
                                LEFT JOIN t_kelas_siswa_reguler c ON CONCAT(a.id_siswa,LEFT(a.tasm,4)) = CONCAT(c.id_siswa,c.ta)
        ...
        $q_siswa_kelas = $this->db->query("SELECT a.id_siswa, b.nama nmsiswa FROM t_kelas_siswa_reguler a LEFT JOIN m_siswa b ON a.id_siswa = b.id WHERE a.id_kelas = '".$pc_bawa[1]."' AND a.ta = '".$this->d['tahun']."'")->result_array();
    ...
    public function import_nilai() {
        ...
        $q_siswa_kelas = $this->db->query("SELECT a.id_siswa FROM t_kelas_siswa_reguler a WHERE a.id_kelas = '".$id_kelas."' AND a.ta = '".$this->d['tahun']."'");
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            $arr_perdata = array();
            foreach ($data as $d) {
                $arr_perdata[] = "('".$d['tasm']."', '".$d['jenis']."', '".$d['id_guru_mapel']."', '".$d['id_mapel_kd']."', '".$d['id_siswa']."', '".$d['nilai']."', '".$d['catatan']."', '".$d['catatan_mid']."')";
            }
    ...
    public function ambil_siswa($kelas) {
        ...
        if($jenis == "c") {
            ...
            $ambil_nilai = $this->db->query("SELECT
                                        ...
                                        INNER JOIN t_kelas_siswa_reguler g ON a.id_siswa = g.id_siswa
    ...
    //         */
    //         $ambil_nilai = $this->db->query("SELECT 
				// 		t_nilai_ket.id_siswa ids, m_siswa.nama, IFNULL(t_nilai_ket.catatan,0) nilai, IFNULL(t_nilai_ket.catatan_mid,0) nilai_mid
				// 		FROM t_nilai_ket 
				// 		INNER JOIN t_guru_mapel ON t_nilai_ket.id_guru_mapel = t_guru_mapel.id
				// 		INNER JOIN m_siswa ON t_nilai_ket.id_siswa = m_siswa.id
				// 		WHERE t_guru_mapel.id = '$id_guru_mapel' AND t_nilai_ket.jenis = '$jenis' AND 
				// 		t_nilai_ket.tasm = '".$this->d['tasm']."'"
				// 		)->result_array();
				$ambil_nilai = $this->db->query("SELECT 
            m_siswa.id AS ids,
            m_siswa.nama,
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            if ($p['jenis'] == 'c'){
                ...
                if ($cek > 0) {
                    $jumlah_sudah ++;
                    $this->db->query("UPDATE t_nilai_ket SET catatan = '$s', catatan_mid = '".$p['nilai_mid'][$i]."' WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_mapel_kd = '".$p['id_mapel_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."'");
                    //$queri[] = "update";
                } else {
                    $this->db->query("INSERT INTO t_nilai_ket (tasm,jenis, id_guru_mapel, id_mapel_kd, id_siswa, catatan, catatan_mid) VALUES ('".$this->d['tasm']."', '".$p['jenis']."', '".$p['id_guru_mapel']."', '".$p['id_mapel_kd']."', '".$p['id_siswa'][$i]."', '".$s."', '".$p['nilai_mid'][$i]."')");
                    //$queri[] = "add";
                }
            }else{
                $cek = $this->db->query("SELECT id FROM t_nilai_ket WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_mapel_kd = '".$p['id_mapel_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."' AND tasm = '".$this->d['tasm']."'")->num_rows();
