...
class Api extends CI_Controller {
    ...
    public function get_monthly_data() {
        ...
        if (empty($nis)) {
            $response = array('error' => 'Invalid nis parameter');
            $this->output
        ...
        // Get id_siswa from m_siswa based on nis
        $id_siswa = $this->db->query("SELECT id FROM m_siswa WHERE nis = '".$nis."'")->row_array();

        if (!$id_siswa) {
            $response = array('error' => 'No student found with nis '.$nis);
            $this->output
                 ->set_content_type('application/json')
                 ->set_output(json_encode($response));
            return;
        ...
        $data = $this->Api_model->get_monthly_data($id_siswa);

        $response = $data ? $data : array('error' => 'No data found for student with nis '.$nis);
